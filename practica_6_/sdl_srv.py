...
# Instancia del selector para socket no bloqueante
...
def read(sock, mask):
    ...
    try:
        ...
        sock.sendall(topic)
        sock.sendall(bytes(tablero, encoding='utf-8'))
        sock.sendall(json.dumps(palabras).encode())

        end = sock.recv(1024)
        # registrar puntaje
        f = open('winners.txt', 'a')
        f.write('Jugador >>> ' + str(addr))
...
def close(sock, addr):
    try:
        ...
        print(str(addr) + ' ya no quiso jugar conmigo :c ...')
